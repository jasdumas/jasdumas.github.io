...
appname <- "ml-flashcards"
## api key (example below is not a real key)
key <- Sys.getenv("RTWEET_KEY")
## api secret (example below is not a real key)
secret <- Sys.getenv("RTWEET_SECRET")
...
```{r}
ml_tweets <- search_tweets("#machinelearningflashcards", n = 500, include_rts = FALSE, token = twitter_token) %>% filter(screen_name == 'chrisalbon')
```
...
```{r, include=FALSE}
# post publishing version of function
save_image <- function(df){
  for (i in c(1:nrow(df))){
    image <- try(image_read(df$media_url[i]), silent = F)
  if(class(image)[1] != "try-error"){
    image %>%
      image_scale("1200x700") %>%
      image_write(paste0("../post_data/data/", ml_tweets$clean_text[i],".jpg"))
  }
